        ...
        <x-filament::card>
            ...
            <table class="filament-tables-table w-full text-left rtl:text-right divide-y table-auto dark:divide-gray-700">
                ...
                <tfoot @class([ 'divide-y whitespace-nowrap' , 'dark:divide-gray-700'=> config('tables.dark_mode'),
                    ...
                    $insurancePays = round($total * ($record->invoice->session->discount->discount / 100), 0);
        ...
        <x-slot name="form">
            ...
            <div id="print-js">
                <x-filament::card>
                    ...
                    <div><span style="font-weight: bold">Invoice N&deg;:</span> PROV-{{ sprintf('%06d', $record->invoice->session->id) }}</div>
                    <div><span style="font-weight: bold">Date:</span> {{ date('d/m/Y', strtotime($record->invoice->session->date)) }}</div>
                    <div><span style="font-weight: bold">Names:</span> {{ $record->invoice->session->fileInsurance->file->names }}</div>
                    ...
                    <div class="flex justify-between">
                        <div>Done by <span style="font-weight: bold">{{ $doneBy }}</span> on <span style="font-weight: bold">{{ date('d/m/Y', strtotime( $record->payments()->count() > 0 ? $record->payments()->latest()?->first()?->created_at : $record->items()->latest()?->first()?->created_at)) }}</span></div>
