    ...
    protected function getCards(): array
    {
        ...
        return [
            ...
            Card::make('Payments', 'RWF ' . (auth()->user()->hasRole('Admin') ? number_format($payments->sum('amount')) : number_format($payments->where('done_by', auth()->id())->sum('amount'))))
                ->description('Cash: ' . (auth()->user()->hasRole('Admin') ? number_format($payments->where('payment_mean_id', 1)->sum('amount')) : number_format($payments->where('done_by', auth()->id())->where('payment_mean_id', 1)->sum('amount'))) . 'RWF, MoMo: ' . (auth()->user()->hasRole('Admin') ? number_format($payments->where('payment_mean_id', 2)->sum('amount')) : number_format($payments->where('done_by', auth()->id())->where('payment_mean_id', 2)->sum('amount'))) . 'RWF')
